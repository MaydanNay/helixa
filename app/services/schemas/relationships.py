...
def get_relationship_context_schema(count):
    ...
    - **Общая история:** Где познакомились, что пережили. Отрази, как их привычки (behavioral) или распорядки (planning) влияют на их общение. Создай 1-3 события (history_events).
    - **Темы:** О чем они обычно говорят (включая общие цели или бытовые привычки).
